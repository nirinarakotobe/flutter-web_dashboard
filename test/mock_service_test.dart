...
void main() {
  group('mock dashboard API', () {
    late DashboardRepository api;

    setUp(() {
      api = DashboardMockData();
    });

    group('items', () {
      test('insert', () async {
        var category = await api.categories.insert(CategoryModel('Coffees Drank'));
        expect(category.name, 'Coffees Drank');
      });

      test('delete', () async {
        await api.categories.insert(CategoryModel('Coffees Drank'));
        var category = await api.categories.insert(CategoryModel('Miles Ran'));
      ...
      test('update', () async {
        var category = await api.categories.insert(CategoryModel('Coffees Drank'));
        await api.categories.update(CategoryModel('Bagels Consumed'), category.id!);
    ...
    group('entry service', () {
      ...
      setUp(() async {
        category = await api.categories.insert(CategoryModel('Lines of code committed'));
      });

      test('insert', () async {
        var entry = await api.entries.insert(category.id!, EntryModel(1, dateTime));
      ...
      test('delete', () async {
        await api.entries.insert(category.id!, EntryModel(1, dateTime));
        var entry2 = await api.entries.insert(category.id!, EntryModel(2, dateTime));
      ...
      test('update', () async {
        var entry = await api.entries.insert(category.id!, EntryModel(1, dateTime));
        var updated =
            await api.entries.update(category.id!, entry.id!, EntryModel(2, dateTime));
